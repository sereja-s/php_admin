...
class Settings
{
	...
	{	// метод get() обращается к методу instance() данного класса (этот метод возвращает свойство в которое будет записана (сохранена) ссылка на объект данного класса) и мы можем обратиться к этому объекту и к его свойству, имя которого пришло на вход функции get()
		return self::instance()->$property;
	}

	// определим функцию (метод), которая будет клеять (объединять) свойства например из массива шаблонов и свойства из массива плагина, если такие ему понадобятся (чтобы нам не приходилось дублировать свойства вручную)7
	...
	public function clueProperties($class)
	{
		...
		// так как перед вызовом функции clueProperties() в классе ShopSettings в файле BaseSettings.php, был создан объект данного класса, в этой функции теперь доступно использование ключевого слова $this которая ссылается на объект нашего класса
		// пройдёмся в цикле по объекту нашего класса $this как имя(название) свойства $name и значения свойства $item
		foreach ($this as $name => $item) {
			// на каждой итерации цикла в переменную $property сохраним свойства класса, который мы передали в параметры функции (указываем переданный класс и вызываем у него метод get() на вход которого передаём имя свойства, которое у нас пришло в качестве имени свойства $name)
			$property = $class::get($name);

			// поверка условия: функция is_array() проверяет: является ли массивом то, что приходит ей на вход (т.е. являются ли свойство private $templateArr из файла Settings.php и свойство private $templateArr ($property) из файла ShopSettings.php ($item) массивами)
			...
			if (!$property) {
				// берём наш результирующий массив $baseProperties и в его ячейку $name запишем, то что находится в свойстве основного объекта настроек
	...
	// (в параметры(на вход) ей ничего не передаём, т.к. аргументы (здесь- массивы свойств) передавались на вход при вызове функции arrayMergeRecursive() и уже попали в память) Из памяти их вытащит функция php: func_get_args()
	public function arrayMergeRecursive()
	{
		...
		// в переменную $base сохраним результат работы функции php: array_shift(), которая возвращает первый элемент массива поданного на вход(здесь- массив $namе) и при этом удаляет его из $arrays (здесь- останется только второй элемент массива: массив $property)  
		...
		foreach ($arrays as $array) {
			...
			foreach ($array as $key => $value) {
				// поверка условия: функция is_array() проверяет: является ли массивом то, что приходит ей на вход, т.е. является ли массивами $value и $base (здесь нам нужен его конкретный элемент с таким же именем ($key) как и у массива $array)
				// (т.е. если это одинаковые свойства)
				if (is_array($value) && is_array($base[$key])) {
					// то ничего не будем делать, а рекурсивно вызовем наш метод arrayMergeRecursive()
					// обратимся к массиву $base и его ячейке $key и сохраняем результат работы функции arrayMergeRecursive(), на вход которой подаём: массив $base и его ячейке $key. а также значение $value
